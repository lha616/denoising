...
%     end
    ...
    if dump_output_information == 1,
        ...
        ret = [y z y_est]
        s = sprintf('%s src    noisy    denoised (sigma = %d, PSNR %.2f to %.2f)', ...
            image_name(6:end),sigma, 10*log10(1/mean((y(:)-z(:)).^2)),PSNR)
        figure, imshow(ret); title(s);
        s = sprintf('%s noisy_P%.2f denoised_P%.2f)', ...
            image_name(6:end - 4), 10*log10(1/mean((y(:)-z(:)).^2)),PSNR)
        imwrite(ret, ['result/' s '.png'])
