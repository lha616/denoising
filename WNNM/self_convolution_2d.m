function  [pos_arr]   =  self_convolution_2d(X, par, Neighbor_arr,Num_arr, SelfIndex_arr)
...
Patnorm = 0.5*(vecnorm(Y)).^2;
for  i  =  1 : L
    Patch = X(:,SelfIndex_arr(i));
    Neighbors = X(:,Neighbor_arr(1:Num_arr(i),i));
    Subpatnorm = Patnorm(Patch);
    D = conv2(Neighbors,rot90(reshape(X(:,off),f,[])',2),"valid");
    Dist = Subpatnorm - D(:)    
    [val index] = sort(Dist);
